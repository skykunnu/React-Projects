...
function handleExpense(e){
  ...
  if(total<Number(expenseAmt) || remaining<Number(expenseAmt)){
    ...
    const obj={ExpenseDetail:expenseDetail, ExpenseAmt:Number(expenseAmt)};
    setSpent(spent+Number(expenseAmt));
    setExpense([...expense, obj]);
    setRemaining(expenseAmt===0?total-expenseAmt:remaining-expenseAmt);
    setExpenseDetail('');
    setExpenseAmt('');
  }
}
  ...
  <div className='mx-auto max-w-xl px-10 py-4 border my-3'>
    ...
    <form className='text-center' onSubmit={handleSubmit}>
     <input type="number" placeholder="Enter the budget" name='budget' className='border mx-2 px-2' value={budget} onChange={(e)=>setBudget(e.target.value)} />
     <button className='px-2 border' type='submit'>Set Budget</button>
    </form>
   ...
   <form className='flex flex-col gap-2 w-72 mx-22' onSubmit={handleExpense}>
    <input type="text" name='detail' placeholder='Expense Detail' className='border' value={expenseDetail} onChange={(e)=>setExpenseDetail(e.target.value)} required />
    <input type="number" name='Amount' placeholder='Expense Amount' className='border' value={expenseAmt} onChange={(e)=>setExpenseAmt(e.target.value)} required />
    <button type='submit' className='border'>Add Expense</button>
   </form>
